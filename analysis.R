...
# Associated to "Trait-specific responses to repeated stressor exposure in wild crickets"
...
packages_list <- c("broom", "brms", "cowplot", "dplyr", "ggplot2",
                   "MASS", "posterior", "purrr", "rstan", "tidyr")
...
## Column selection
data_all<- data_all %>%
  select(idx, date, burrow, sex, ball, trial, temp, flee_dis, fs1, emergence, path_dist, n_trials_treatment)
...
emmeanplot <- ggplot() +
  ...
  scale_x_continuous(breaks = 1:10) +
  scale_y_continuous(limits = c(0, 550), breaks = seq(0, 550, by = 25)) +
  scale_color_manual(values = custom_colours) +
  theme_classic() +
  theme(
    axis.ticks = element_line(color = "black"),
...
## Remove legends, include one across all plots
emmeanplot <- emmeanplot + theme(legend.position = "none")
fsmeanplot <- fsmeanplot + theme(legend.position = "none")
dfbmeanplot <- dfbmeanplot + theme(legend.position = "none")

legend_plot <- ggplot(data_all, aes(x = trial, y = emergence, color = stimulus)) +
  geom_point() +
  scale_color_manual(
    name = "Stimulus\nintensity",
...
top <- emmeanplot
bottom <- plot_grid(fsmeanplot, dfbmeanplot, ncol = 2, labels = c("B", "C"))

mean_indiv_resp_stimulustype_plot <- plot_grid(
  plot_grid(top, bottom, ncol = 1, rel_heights = c(2, 1), labels = c("A", "")),
  legend,
  ncol = 2,
  rel_widths = c(1, 0.1),
  align = "h"
